...
class LockState:
    ...
    def release_and_grant_next(self, txn_id) -> list[int]:
        ...
        while self.queue:
            next_txn, requested_mode = self.queue[0]

            if requested_mode == LockMode.SHARED:
                if self.lock_mode != LockMode.EXCLUSIVE:
                    self.queue.popleft()
                    self.holders.add(next_txn)
                    self.lock_mode = LockMode.SHARED
                    unblocked.append(next_txn)
                else:
                    break

            elif requested_mode == LockMode.EXCLUSIVE:
                if self.lock_mode == LockMode.NONE or self.holders == {next_txn}:
                    self.queue.popleft()
                    self.holders.add(next_txn)
